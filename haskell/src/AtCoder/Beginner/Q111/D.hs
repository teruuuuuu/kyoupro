module AtCoder.Beginner.Q111.D where
...
odd_even a = let (h:t) = (map (\(x,y)->mod(x+y) 2) a) in
  (and (map (h==) t),h==0)
...
adj_solver n (x,y) = if mod (x+y) 2 == 1
                          then solver n (x,y)
...
solver_test n ls = let fl = zip [0..n-1] ls in
                   let x = sum [if c == 'R' then 2^i else (-(2^i)) | (i,c) <-fl, c=='L'||c=='R'] in
                   let y = sum [if c == 'U' then 2^i else (-(2^i)) | (i,c) <-fl, c=='U'||c=='D'] in
                   (x, y)
